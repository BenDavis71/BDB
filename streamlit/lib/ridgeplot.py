...
def draw_ridgeplot(dfs, names, colors, group_count):
    ...
    fig.update_traces(orientation='h', side='negative', width=2.8, points=False, bandwidth=.2, 
        meanline_visible=True, meanline=dict(color='white',width=1.5), hoveron='kde', spanmode='manual',span=[-3,3])
    fig.update_layout(xaxis_title='EPA',xaxis_showgrid=False, xaxis_zeroline=False, showlegend=False,  yaxis_autorange='reversed', height=(280+40*group_count))
